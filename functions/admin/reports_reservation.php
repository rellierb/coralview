...
if(isset($_POST["reference_no"])) {
    $main_reference_no = mysqli_real_escape_string($db, trim($_POST['reference_no']));
}
...
if(!empty($main_reference_no)) {

    $reservation_details_query = "SELECT * FROM reservation RES
    INNER JOIN guest G ON G.id = RES.guest_id
    WHERE RES.reference_no = '$main_reference_no' AND RES.date_created BETWEEN '$format_date_reservation_from' AND '$format_date_reservation_to'";

} else if(empty($main_reference_no)) {

    $reservation_details_query = "SELECT * FROM reservation RES
    INNER JOIN guest G ON G.id = RES.guest_id
    WHERE RES.date_created BETWEEN '$format_date_reservation_from' AND '$format_date_reservation_to'";
...
if(mysqli_num_rows($reservation_details_result) > 0) {
    ...
    while($reservation = mysqli_fetch_assoc($reservation_details_result)) {
        ...
        if(empty($main_reference_no)) {
            ...
            $reference_no = $main_reference_no;
        }
            ...
            while($room_reservation = mysqli_fetch_assoc($room_reservation_details_result)) {
                ...
                $html .= '
                    <tr>
                        ...
                        <td class="tg-0pky" colspan="4" style="text-align: center;">' . number_format($room_rate, 2) . '</td>
                    </tr>
                ';
    ...
    header("location: ../../admin/reservation_report.php"); 
