...
if(!empty($reference_no)) {
    ...
    WHERE RES.reference_no = '$main_reference_no' AND RES.date_created BETWEEN '$date_reservation_from' AND '$date_reservation_to'";

} else if(empty($reference_no)) {

    $reservation_details_query = "SELECT * FROM reservation RES
    INNER JOIN guest G ON G.id = RES.guest_id
    WHERE RES.date_created BETWEEN '$date_reservation_from' AND '$date_reservation_to'";
...
if(mysqli_num_rows($reservation_details_result) > 0) {
    ...
    header("location: ../../admin/reports/reservation_report.php"); 
